        ...
        <div className="flex flex-col h-full">
            <div className="flex flex-col flex-grow">
                ...
                <NavLink to="/members" className={`flex flex-row items-center px-4 justify-between h-16 ${location.pathname === '/members' ? 'bg-blue-600 hover:text-white' : 'bg-slate-50 hover:bg-blue-100'}`}>
                    <img src={location.pathname === "/members" ?  MembersLogo : MembersLogoBlue } alt="logo" className="w-8 h-8" />
                ...
                <NavLink to="/add" className={`flex flex-row items-center px-4 justify-between h-16 ${location.pathname === '/add' ? 'bg-blue-600 hover:text-white' : 'bg-slate-50 hover:bg-blue-100'}`}>
                    <img src={location.pathname === "/add" ? AddUserLogoBlue : AddUserLogo} alt="logo" className="w-8 h-8" />
                    <div className={`text-xl font-bold ${location.pathname === "/add" ? 'text-white' : 'text-blue-500'}`}>إضافة عضو جديد</div>
                </NavLink>
                <NavLink to="/billing" className={`flex flex-row items-center px-4 justify-between h-16 ${location.pathname === '/billing' ? 'bg-blue-600 hover:text-white' : 'bg-slate-50 hover:bg-blue-100'}`}>
                    <img src={location.pathname === "/billing" ? BillLogoWhite : BillLogoBlue} alt="logo" className="w-8 h-8" />
                    <div className={`text-xl font-bold ${location.pathname === "/billing" ? 'text-white' : 'text-blue-500'}`}>سجل الفواتير</div>
                </NavLink>
            </div>
            <div>
                <NavLink 
